...
const HomePage = ({ onNavigate, isLoggedIn, userFirstName }: HomePageProps) => {
  ...
  const [sessionId] = useState(`homepage_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`);
  ...
  const handleChatSend = async () => {
    ...
    try {
      ...
      const recommendationKeywords = ['recommend', 'course', 'class', 'interested in', 'looking for', 'want to learn', 'study'];
      ...
      if (isAskingForRecommendations) {
        ...
        const previousMessages = chatMessages.slice(-3); // Get last 3 messages for context
        const computerScienceContext = previousMessages.some(msg => 
          ...
          msg.content.toLowerCase().includes('coding')
        );
        ...
        try {
          ...
          if (response.recommendations && response.recommendations.length > 0) {
            ...
            botResponse = "I couldn't find specific courses matching your interests, but I'd be happy to help you explore the course catalog! Try searching for specific subjects or majors.";
          }
        } catch (error) {
          console.log('AI recommendation failed, using fallback response:', error);
          botResponse = "I'm having trouble with recommendations right now, but I can help you explore courses! Try asking about specific subjects like Computer Science, Math, Economics, Biology, or Psychology.";
        }
      } else {
        // Use general AI chat
        try {
          ...
          console.log('AI chat failed, using fallback response:', error);
          botResponse = "I'm here to help you find courses! 🎓 Try asking about specific subjects, majors, or what you're interested in learning.";
        }
      }
